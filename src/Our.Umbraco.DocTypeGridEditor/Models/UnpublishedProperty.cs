...
namespace Our.Umbraco.DocTypeGridEditor.Models
{
    internal class UnpublishedProperty : IPublishedProperty
    {
        ...
        public UnpublishedProperty(IPublishedPropertyType propertyType, object value)
        {
            ...
            this.objectValue = new Lazy<object>(() => this.propertyType.ConvertInterToObject(null, PropertyCacheLevel.None, this.sourceValue.Value, true));
            this.xpathValue = new Lazy<object>(() => this.propertyType.ConvertInterToXPath(null, PropertyCacheLevel.None, this.sourceValue.Value, true));
        }
